...
export class FilterPipeUser implements PipeTransform {
    ...
    transform(value: any, args1: any): any {
        ...
        for (const usuario of value) {
            if ( (usuario.id.toString().toLowerCase().indexOf(args1[0].toLowerCase()) > -1)
            ...
            && (usuario.apellidoPaterno.toLowerCase().indexOf(args1[5].toLowerCase()) > -1)   
            && (usuario.apellidoMaterno.toLowerCase().indexOf(args1[6].toLowerCase()) > -1)
            )   {
                 if (args1[1] === '') {
